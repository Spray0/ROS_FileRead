...
// 读取pgmP5文件
 bool pgmP5::LoadFile(char* path){
	 ...
	 if(path==NULL)return false;	 //如果输入的参数为空则退出

	 pgmP5::pFile = fopen (path,"rb");	//打开文件

	 if (pgmP5::pFile==NULL) return false;
	 else{
		 fseek (pgmP5::pFile, 0, SEEK_END);	 pgmP5::FileSize_Bytes=ftell (pgmP5::pFile);	//打开文件成功 获取文件大小
		 ...
		 if(fread(pgmP5::__DPtr,1,pgmP5::FileSize_Bytes,pgmP5::pFile)==pgmP5::FileSize_Bytes){
			 ...
			 if(Get_INFO()==false) return false;
			 return true;
		 }
		 return false;
	 }
	...
	if((*pgmP5::__DPtr=='P')&&(*(pgmP5::__DPtr+1)=='5')&&(*(pgmP5::__DPtr+2)=='\n')){
		...
		if(*(pgmP5::__DPtr+3)=='#'){	//如果这行是信息
			...
			//文件信息存储
			 ...
			 for (int c=0;c<temp-3;c++)*(pgmP5::__INFO_Ptr+c)=*(pgmP5::__DPtr+3+c);
			 temp=temp+1;
		}

		//尺寸信息

		char buf[10];char bufcnt=0;
		while((*(pgmP5::__DPtr+temp)!=' ')||(temp!=pgmP5::FileSize_Bytes)){
			...
			temp=temp+1;	}
		if(temp==pgmP5::FileSize_Bytes)return false;
		//存储宽度
		pgmP5::Size.width=atoi(buf);
		temp=temp+1;

		char buf2[10]={};char bufcnt2=0;
		...
		//检查完整性
		if(temp+(unsigned long)(pgmP5::Size.length*pgmP5::Size.width)!=pgmP5::FileSize_Bytes)	return false;
