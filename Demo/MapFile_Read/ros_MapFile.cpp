...
bool ROS_MapFile::Load(ros::NodeHandle n) {
	...
	ROS_MapFile::origin.x = origin_temp[0];
	ROS_MapFile::origin.y = origin_temp[1];
	ROS_MapFile::origin.yaw = origin_temp[2];
	...
	ROS_MapFile::size.length = MapPGM->Size.length;
	ROS_MapFile::data.len = MapPGM->Size.width * MapPGM->Size.length;
